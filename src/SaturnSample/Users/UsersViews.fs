...
module Views =
  let index (ctx : HttpContext) (objs : User list) =
    let cnt = [
      div [_class "container "] [
        h2 [ _class "title"] [rawText "Listing Users"]
        ...
        a [_class "button is-text"; _href (Links.add ctx )] [rawText "New User"]
      ]
    ]
  ...
  let show (ctx : HttpContext) (o : User) =
    let cnt = [
      div [_class "container "] [
        h2 [ _class "title"] [rawText "Show User"]

        ul [] [
          li [] [ strong [] [rawText "Id: "]; rawText o.id ]
          li [] [ strong [] [rawText "Password: "]; rawText o.password ]
          li [] [ strong [] [rawText "Login: "]; rawText o.login ]
        ]
        a [_class "button is-text"; _href (Links.edit ctx o.id)] [rawText "Edit"]
    ...
    let cnt = [
      div [_class "container "] [
        form [ _action (if isUpdate then Links.withId ctx o.Value.id else Links.index ctx ); _method "post"] [
          ...
          yield field (fun i -> i.id) "Id" "id" 
          yield field (fun i -> i.password) "Password" "password" 
          yield field (fun i -> i.login) "Login" "login" 
          yield buttons
        ]
      ]
    ]
